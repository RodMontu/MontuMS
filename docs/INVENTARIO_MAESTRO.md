...
- Protocolo: `http2`
- Config: `/etc/cloudflared/config.yml` (real, en serverX; `/srv/cloudflared/config.yml` es symlink al anterior — antes vivía en serveri3, migrado)
...
- serverX: configuración de `cloudflared` (`/etc/cloudflared/config.yml` + credenciales; `/srv/cloudflared/config.yml` es symlink)
- serveri3: configuración `pihole` (volúmenes), config `nginx`.
...
**Wrapper `~/bin/Carlitos` (política fovea selectivo, act. 2026-08-19):** default
fovea OFF (`--exclude-tools` sobre las 4 tools de fovea), flag `--fovea` como
primer argumento la activa para tareas multi-archivo. Detalle del mecanismo y
justificación en `docs/EVALUACION_HARNESS_AGENTICOS_CARLITOS_2026-08.md`. Copia
versionada del script en `harness/carlitos/wrapper_carlitos.sh` — `~/bin` en el
Mac Studio NO está bajo git, es el único respaldo en el repo.
...
- **BACKLOG-MODEL-SWAP-BENCH [CERRADO 2026-08-20]:** Benchmark de swap de modelo ejecutado sobre 15 tareas reales de OptiFierro-V2 con fovea-selectivo por tarea, comparando Qwen3-Coder-30B-A3B-Instruct (MoE) vs Devstral-Small-2-24B-Instruct-2512 (denso). Ambos candidatos 15/15 OK, tool-calling verificado. Resultado: MoE 68.0s/tarea promedio (~17.2 tok/s salida) vs denso 106.9s/tarea promedio (~9.7 tok/s salida) — confirma que el modelo denso es más lento pese a tener menos parámetros totales (24B vs 30B). Ningún candidato reemplazó a producción en esta pasada (decisión pendiente de evaluación de calidad, fuera de alcance de este benchmark). El intento nocturno previo (2026-08-19) murió sin completar nada más allá de la descarga del candidato 1, por un bug de razonamiento del agente (creyó que existía un mecanismo de notificación en background que no existe en invocaciones headless de un solo turno) — ver LOG_CAMBIOS_2026.md (2026-08-19/20) para el RCA completo. Detalle en `docs/evidencia/REPORTE_BENCHMARK_MODEL_SWAP_2026-08-19.md`.
- **BACKLOG-BIBLIOTECA-PATHS [CERRADO 2026-08-19 - diagnostico corregido, no era el bug que parecia]:** RCA mas profundo revelo que NO era un bug de `indexador.py`: los archivos duplicados de raiz (`MontuMS/LOG_CAMBIOS_2026.md`, `MontuMS/INVENTARIO_MAESTRO.md`) ya habian sido borrados del disco (ver COMO_USAR_LA_BIBLIOTECA.md seccion 7), y las filas "duplicadas" en el catalogo eran huerfanas de esos archivos ya inexistentes -- se limpiaron solas via `limpiar_huerfanos()` en el reindex de la sesion 2026-08-19 (parte 1, G2). Confirmado con query directa: hoy el catalogo solo tiene `docs/LOG_CAMBIOS_2026.md` y `docs/INVENTARIO_MAESTRO.md`, sin duplicados. El item original habia mezclado esto con el problema real de `docs/evidencia/` no cubierta, que era un bug distinto (ver BACKLOG-BIBLIOTECA-APPLEDOUBLE, cerrado en el mismo commit, para el detalle del fix de recursividad).
- **BACKLOG-BIBLIOTECA-APPLEDOUBLE [CERRADO 2026-08-19]:** Corregido en `indexador.py`: se filtran archivos con prefijo `._` antes de indexar (`encontrados = [p for p in encontrados if not p.name.startswith("._")]`). Ademas se aprovecho el mismo cambio para hacer el scan de `docs/` recursivo (`SCAN_DIRS_RECURSIVE`), cerrando tambien la cobertura faltante de `docs/evidencia/`. Verificado: reindex post-fix elimino las 4 filas huerfanas previas, cero filas AppleDouble nuevas, y `docs/evidencia/REPORTE_BENCHMARK_FOVEA_OPTIFIERRO_2026-08-18.md` ahora indexado (3 secciones). Backup pre-fix en `/tmp/indexador.py.pre-fix-2026-08-19.bak` en serverX.
