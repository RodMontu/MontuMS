...
- **BACKLOG-BIBLIOTECA-APPLEDOUBLE:** El reindex de La Biblioteca (2026-08-19, ver LOG_CAMBIOS) indexo 4 archivos basura AppleDouble de macOS (prefijo `._`, ej. `docs/._INVENTARIO_MAESTRO.md`) generados por Samba/Finder al sincronizar con /mnt/extra. `indexador.py` no filtra estos nombres. RCA pendiente: agregar exclusion de patron `._*` al glob de indexador.py antes del proximo reindex. No borrar las filas basura manualmente hasta tener el fix (evitar reintroducirlas en el siguiente reindex sin el filtro).
